...
start = time.time()
count = 0
update = todo_store.push_and_play if rebase_db else todo_store.push
for event in events:
    update(Event.create(json.loads(event)))

    if rebase_db and count % 500 == 0:
        session.commit()
    count += 1
else:
    session.commit()
